...
class MinesweeperAI():
        ...
        for i in range(cell[0] - 1, cell[0] + 2):
            for j in range(cell[1] - 1, cell[1] + 2):
                if (i,j) in self.mines:
                    cmines += 1
                if i in range(0,self.height) and j in range(0,self.width) and (i,j) not in self.safes and (i,j) not in self.mines:
                    new_cells.add((i,j))
        new_know=Sentence(new_cells,count-cmines)
    ...
    def update_knowledge(self):
        ...
        while(changed):
            ...
            #finding new mines and updating the sentences
            changed = False
            new_mines=set()
            new_safes=set()
            for sentence in self.knowledge:
                ...
                for sentence2 in self.knowledge:
                    if sentence1 != sentence2 and sentence2.cells.issubset(sentence1.cells) and sentence2.cells:
                        ...
                        if newcount >=0:
                            ...
                            if new_sentence not in new_sentences and new_sentence not in self.knowledge:
        ...
        for i in range(0,self.height):
            for j in range(0,self.width):
                if (i,j) not in self.moves_made and (i,j) not in self.mines:
                    moves.add((i,j))
        if len(moves)>0:
            random_cell = random.choice(list(moves))
